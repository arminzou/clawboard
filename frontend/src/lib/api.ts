...
export const api = {
  async health() {
    return json<{ status: string; timestamp: string }>(await fetch(`${API_BASE}/api/health`));
  },
  ...
  async listTasks(params?: { status?: TaskStatus; assigned_to?: string }) {
    ...
    const url = `${API_BASE}/api/tasks${usp.toString() ? `?${usp.toString()}` : ''}`;
  ...
  async listActivities(params?: { agent?: string; limit?: number }) {
    ...
    const url = `${API_BASE}/api/activities${usp.toString() ? `?${usp.toString()}` : ''}`;
