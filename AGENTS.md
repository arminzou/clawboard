...
- `frontend/`: React + TypeScript + Vite UI.
  - `src/pages/`: page-level features (Kanban, Activity, Inbox, Docs, Settings).
  - `src/components/`: shared UI and layout components.
- `extensions/pawvy/`: OpenClaw plugin for real-time agent presence tracking.
...
- `pnpm -C backend test:run`: run backend Vitest suite.
...
- Follow Conventional Commit style:
  - `feat(tasks): add anchor field to task creation`
  - `fix(api): restore assignee validation on bulk update`
...
- API auth uses `PAWVY_API_KEY`; frontend reads it from build args or `frontend/.env.local`.
- Webhook endpoint (`/api/webhook/pawvy`) is intentionally exempt from API key auth.
- For task management, update task state via API endpoints — never direct DB edits.
